...
loadLibrary :: proc(
    ...
    loc: core.Location,
    lib_path: string
) -> (lib: dynlib.Library, ok: bool) {
    lib, ok = dynlib.load_library(lib_path)
    if !ok {
        reportError(intr, loc, "failed to load library with path '%s'", lib_path) or_return
    }
    return lib, true
}

ainsStructTypeToFFIStructType :: proc(
    intr: ^Interpreter,
    s: ^core.Struct,
    loc: core.Location
) -> (t: ^ffi.ffi_type, ok: bool) {
    ...
    for field, i in s.fields {
        elems[i] = ainsTypeStringToFFIType(intr, field.value.(core.String), loc) or_return // FIXME: 
    }
    return struct_type, true
}

ainsTypeStringToFFIType :: proc(
    intr: ^Interpreter,
    type: string,
    loc: core.Location
) -> (t: ^ffi.ffi_type, ok: bool) {
    ...
    case:
        reportError(intr, loc, "unhandled type '%s'", type) or_return
        return {}, false
    }
}
...
convertCValueToASValue :: proc(
    intr: ^Interpreter,
    loc: core.Location,
    c_val: rawptr,
    target_type: ^ffi.ffi_type
) -> (res: core.Value, ok: bool) {
    ...
    reportError(intr, loc, "can't convert C type '%s' to AinScript value", ffi.getTypeName(target_type.type)) or_return
    return {}, false
}
...
convertASValueToCValuePtr :: proc(
    ...
    switch v in value {
    case core.Module:
        reportError(intr, loc, "can't convert AinScript module to any C value") or_return
    case core.Number:
        ...
        case ffi.FFI_TYPE_UINT32:
            ...
            if 0 <= val {
                ...
                reportError(intr, loc, "can't convert negative number to 'u32'") or_return
            }
        case ffi.FFI_TYPE_UINT8:
            ...
            if 0 <= val {
                ...
                reportError(intr, loc, "can't convert negative number to 'u8'") or_return
            }
        case:
            reportError(intr, loc, "can't convert AinScript number to '%s'", ffi.getTypeName(ffi_target_type.type)) or_return
        }
    case ^core.Func:
        reportError(intr, loc, "can't convert AinScript function to any C value") or_return
    case core.Nil:
        reportError(intr, loc, "can't convert AinScript nil to any C value") or_return
    case core.Bool:
        switch ffi_target_type.type {
        case ffi.FFI_TYPE_SINT32:
            ptr := new(i32, context.temp_allocator)
        ...
        case ffi.FFI_TYPE_UINT32:
            ...
            return rawptr(ptr), true
        case:
            reportError(intr, loc, "can't convert AinScript string to '%s'", ffi.getTypeName(ffi_target_type.type)) or_return
        }
    case core.String:
        switch ffi_target_type.type {
        case ffi.FFI_TYPE_POINTER: 
            ...
            return rawptr(ptr), true
        case:
            reportError(intr, loc, "can't convert AinScript bool to c '%s'", ffi.getTypeName(ffi_target_type.type)) or_return
        }
    case ^core.Array:
    case ^core.Struct:
        len := 0
        ...
        for field, i in v.fields {
            ...
            converted_field := convertASValueToCValuePtr(intr, loc, field.value, ffi_type) or_return
